...
export async function bulkCreatePosts(
  ...
  try {
    ...
    revalidatePath("/admin/posts")
    revalidatePublicPostRoutesIfNeeded(...rawPosts)
...
export async function updatePost(
  ...
  try {
    ...
    await recordAudit({
      actorEmail: session.email,
      entityType: "Post",
      entityId: post.slug, // Keep tracking by slug in audit or switch to ID? Slug is more readable.
      action: "UPDATE",
...
export async function deletePost(id: number): Promise<ActionResult<{ id: number }>> {
  if (!prisma) {
    return { ok: false, message: "Database is not configured." }
  }
